...
date = input('\nEnter Vaccine Tracking Date(DD/MM/YYYY format only) => ')
date = (date.replace('/', ''))
dist_code = input("\nEnter District code => ")
age = int(input('\nEnter age => '))
...
# In[ ]:
    ...
    while True:
        ...
        print("\nSearching for vaccine(Dose 1), trial number :{} at {}".format(count, now))
        response = requests.get(URL, headers=header)
        if response.ok:
            response_json = response.json()
            response_df = pd.DataFrame(dict(response_json)['sessions'])
            filter_df = response_df[(response_df.min_age_limit == age)
                                    & (response_df.available_capacity_dose1 > vaccine_limit)]
            from tkinter import *
            ...
            if filter_df.shape[0] > 0:
                ...
                webbrowser.open("https://selfregistration.cowin.gov.in/", new=1)
                for i in range(filter_df.shape[0]):
                    Data = "Time {}, Centre Name : {}, Address : {}, Pin :{}, Total dose :{}, Vaccine Type :{}".format(now,
                        filter_df.name.iloc[i], filter_df.address.iloc[i],
